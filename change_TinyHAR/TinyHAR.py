...
class SelfAttention_interaction(nn.Module):
    ...
    def __init__(self, sensor_channel, n_channels):
        ...
        self.query         = qnn.QuantLinear(n_channels, n_channels, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.key           = qnn.QuantLinear(n_channels, n_channels, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.value         = qnn.QuantLinear(n_channels, n_channels, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        ...
        self.net = nn.Sequential(
            qnn.QuantLinear(dim, hidden_dim, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH),
            qnn.QuantReLU(),
            qnn.QuantDropout(dropout),
            qnn.QuantLinear(hidden_dim, dim, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH),
            qnn.QuantDropout(dropout)
        )
    def forward(self, x):
        return self.net(x)
...
class Attention(nn.Module):
    def __init__(self, dim, heads = 4, dim_head = 16, dropout = 0.):
        ...
        self.dropout = qnn.QuantDropout(dropout)

        self.to_qkv = qnn.QuantLinear(dim, inner_dim * 3, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)

        self.to_out = nn.Sequential(
            qnn.QuantLinear(inner_dim, dim, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH),
            qnn.QuantDropout(dropout)
        ) if project_out else qnn.QuantIdentity(bit_width = ACT_BIT_WIDTH)
...
class seperate_FC_interaction(nn.Module):
    def __init__(self, sensor_channel, filter_num):
        ...
        self.fc_filter = qnn.QuantLinear(filter_num ,filter_num,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.fc_channel  = qnn.QuantLinear(sensor_channel ,sensor_channel,bias=True)
    def forward(self,x):
        # input b C F 
        #x = x.permute(0,1,3,2)
...
class FilterWeighted_Aggregation(nn.Module):
    ...
    def __init__(self, sensor_channel, n_channels):
        ...
        self.value_projection = qnn.QuantLinear(n_channels, n_channels,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.value_activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 
        
        self.weight_projection = qnn.QuantLinear(n_channels, n_channels,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.weighs_activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 
...
class NaiveWeighted_Aggregation(nn.Module):
    ...
    def __init__(self, sensor_channel, hidden_dim):
        ...
        self.fc = qnn.QuantLinear(hidden_dim, 1, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
...
class Weighted_Aggregation(nn.Module):
    ...
    def __init__(self, sensor_channel, hidden_dim):
        ...
        self.weight_projection = qnn.QuantLinear(hidden_dim, hidden_dim,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.weighs_activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 
        self.fc = qnn.QuantLinear(hidden_dim, 1,bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.sm = torch.nn.Softmax(dim=1)
...
class FC(nn.Module):
    ...
    def __init__(self, channel_in, channel_out):
        ...
        #change to bias = False
        self.fc = qnn.QuantLinear(channel_in ,channel_out,bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
...
class seperate_FC_channel_first(nn.Module):
    def __init__(self, sensor_channel, filter_num):
        ...
        self.fc_channel = qnn.QuantLinear(sensor_channel ,1,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.fc_filter  = qnn.QuantLinear(filter_num ,filter_num,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
    def forward(self,x):
        # input b L C F 
        x = x.permute(0,1,3,2)
...
class seperate_FC_filter_first(nn.Module):
    def __init__(self, sensor_channel, filter_num):
        ...
        self.fc_filter = qnn.QuantLinear(filter_num ,1,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.fc_channel  = qnn.QuantLinear(sensor_channel ,filter_num, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 
    def forward(self,x):
        # input b L C F 
        #x = x.permute(0,1,3,2)
...
class seperate_FC_filter_first_v2(nn.Module):
    def __init__(self, sensor_channel, filter_num):
        ...
        self.fc_filter_1 = qnn.QuantLinear(filter_num ,filter_num, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.fc_channel_1  = qnn.QuantLinear(sensor_channel ,sensor_channel, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 

        self.fc_filter_2 = qnn.QuantLinear(filter_num,1,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.fc_channel_2  = qnn.QuantLinear(sensor_channel ,filter_num, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
...
class FC_Weighted_Aggregation(nn.Module):
    ...
    def __init__(self, sensor_channel, hidden_dim):
        ...
        self.fc_filter_1 = qnn.QuantLinear(hidden_dim ,hidden_dim, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.fc_channel_1  = qnn.QuantLinear(sensor_channel ,sensor_channel, bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 
...
class temporal_LSTM(nn.Module):
    ...
    def __init__(self, sensor_channel, filter_num):
        ...
        self.lstm = qnn.QuantLSTM(filter_num, 
                            filter_num, 
                            batch_first =True)
    def forward(self, x):
        # Batch length Filter
    ...
    def __init__(self, sensor_channel, filter_num, nb_layers=2):
        ...
        for i in range(nb_layers):
            ...
            layers_conv.append(nn.Sequential(
                qnn.QuantConv1d(in_channel, out_channel, 5, bias=False,weight_bit_width = WEIGHT_BIT_WIDTH),
...
class Temporal_Weighted_Aggregation(nn.Module):
    ...
    def __init__(self, sensor_channel, hidden_dim):
        ...
        #chage bias to false
        self.fc_1 = qnn.QuantLinear(hidden_dim, hidden_dim, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.weighs_activation = qnn.QuantReLU(bit_width = ACT_BIT_WIDTH) 
        self.fc_2 = qnn.QuantLinear(hidden_dim, 1, bias=False, weight_bit_width = WEIGHT_BIT_WIDTH)
        self.sm = torch.nn.Softmax(dim=1)
        self.gamma         = nn.Parameter(torch.tensor([0.]))
...
class TinyHAR_Model(nn.Module):
    def __init__(
        ...
        """
        PART 0 FINN expect integer input     
        """
        self.quant_8 = qnn.QuantIdentity(bit_width = 8)
        self.quant_4 = qnn.QuantIdentity(bit_width = 4)
        ...
        for i in range(nb_conv_layers):
            ...
            if i%2 == 1:
                layers_conv.append(nn.Sequential(
                    qnn.QuantConv2d(in_channel, out_channel, (filter_size, 1),(2,1),bias=False, weight_bit_width = 4),
                    qnn.QuantReLU(bit_width = 4),#))#,
                    nn.BatchNorm2d(out_channel),
                    qnn.QuantIdentity(bit_width = 4)))
            else:
                layers_conv.append(nn.Sequential(
                    qnn.QuantConv2d(in_channel, out_channel, (filter_size, 1),(1,1),bias=False, weight_bit_width = 4),
                    qnn.QuantReLU(bit_width = 4),#))#,
                    nn.BatchNorm2d(out_channel),
                    qnn.QuantIdentity(bit_width = 4)))
        self.layers_conv = nn.ModuleList(layers_conv)
        ...
        self.decoy= self.get_reshape(input_shape) 
        ...
        # PART 6 , ==================== Prediction ==============================
        self.prediction = qnn.QuantLinear(2*filter_num ,number_class,bias=True, weight_bit_width = WEIGHT_BIT_WIDTH)
    ...
    def get_reshape(self, input_shape):
        x = torch.rand(input_shape)
    ...
    def forward(self, x):
        # B F L C   
        batch = torch.rand(x.shape).shape[0]
        ...
        if self.cross_channel_aggregation_type == "FC":
            ...
            x = x.permute(0, 3, 1, 2)
            x = x.reshape(batch, self.decoy, -1)
